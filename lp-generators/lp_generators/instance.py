...
class DenseLHS(object):
    ...
    def __init__(self, lhs, variable_types=None, **kwargs):
        super().__init__(**kwargs)
        if isinstance(lhs, sparsemat.spmatrix):
            lhs = lhs.todense()
        self._lhs_matrix = np.matrix(lhs, dtype=np.float)
        # Check for empty variables which are likely to break things later.
        empty_columns = ((lhs != 0).sum(axis=0) == 0)
        if empty_columns.any():
            raise ValueError(f"LHS matrix has empty columns: {np.where(empty_columns)[1].tolist()}")
        # Variable typing defaults to a pure LP.
        if variable_types is None:
            self._variable_types = "C" * lhs.shape[1]
        else:
            assert type(variable_types) is str and len(variable_types) == lhs.shape[1]
